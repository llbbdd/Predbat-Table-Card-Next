...
class PredbatTableCard extends HTMLElement {
    ...
    */
        ...
        return {
            ...
            computeLabel: (schema) => {
                if (schema.name === "entity")
                    return "Predbat HTML Entity:";
                if (schema.name === "fill_empty_cells")
                    return "Fill Empty Cells?";
                if (schema.name === "show_day_totals")
                    return "Show Day Totals Row?";
                if (schema.name === "show_plan_totals")
                    return "Show Plan Totals Row?";
                if (schema.name === "use_friendly_states")
                    return "Use friendly STATE labels?";
                if (schema.name === "stack_pills")
                    return "Stack Import/Export Pills?";
                if (schema.name === "old_skool")
                    return "Use original Predbat Plan stylesheet? (old_skool mode)";
                if (schema.name === "old_skool_columns")
                    return "Choose specific columns to use original Predbat Plan style";
                if (schema.name === "help_text")
                    return "Important: You must manually set the columns in your card YAML";
                if (schema.name === "table_width")
                    return "Table Width (%)";
                if (schema.name === "font_size")
                    return "Font Size (px)";
                if (schema.name === "row_limit")
                    return "Number of rows to return";
                if (schema.name === "show_predbat_version")
                    return "Show Predbat version?";
                if (schema.name === "show_tablecard_version")
                    return "Show Predbat Table Card version?";
                if (schema.name === "hide_last_update")
                    return "Hide PLAN LAST UPDATED header?";
                if (schema.name === "hide_empty_columns")
                    return "Hide empty columns?";
                if (schema.name === "battery_capacity")
                    return "Battery Capacity";
                if (schema.name === "color_help_text")
                    return "Row colour override settings";
                if (schema.name === "color_help_text_more")
                    return "Override the HEX (e.g, #AA0000) colour values of the rows";
                if (schema.name === "light_mode")
                    return "Card Light Mode";
                if (schema.name === "odd_row_colour")
                    return "Dark Row Colour (odd)";
                if (schema.name === "odd_row_colour_light")
                    return "Light Row Colour (odd)";
                if (schema.name === "even_row_colour")
                    return "Dark Row Colour (even)";
                if (schema.name === "even_row_colour_light")
                    return "Light Row Colour (even)";
                if (schema.name === "debug_prices_only")
                    return "Show Debug Prices Only?";
                if (schema.name === "weather_entity")
                    return "Weather Entity";
                if (schema.name === "path_for_click")
                    return "Dashboard Path for click";
                if (schema.name === "reset_day_totals")
                    return "Reset Total Cost at midnight?";
                return undefined;
            },
    ...
    set hass(hass) {
      ...
      `;
        ...
        if (this.config === undefined)
            throw new Error("this.config not set");
    ...
    async subscribeForecast() {
        if (this.unsubscribe || !this._hass)
            return;
        try {
            this.unsubscribe = await this._hass.connection.subscribeMessage((event) => {
                this.forecast = event.forecast || [];
                if (this._hass === undefined)
                    throw new Error("_hass undefined");
    ...
    async processAndRender(hass) {
        ...
        columnsToReturn = columnsToReturn.filter(column => {
            ...
            if (firstDataArray === undefined)
                throw new Error("firstDataArray is undefined");
        ...
        dataArray.forEach((item, index) => {
            ...
            let currentCost;
            columnsToReturn.forEach((column, columnIndex) => {
                if (item[column] !== undefined) {
                    if (item["time-column"] === undefined)
                        throw new Error("item['time - column'] is undefined");
                    ...
                    if (newColumn === undefined)
                        throw new Error("newColumn undefined");
                    ...
                    if (column === "options-column" || column === "options-popup-column") {
                        if (useRefactor) {
                            if (item[column] === undefined)
                                throw new Error("item[column] is undefined");
                            if (item["time-column"] === undefined)
                                throw new Error("item['time - column'] is undefined");
                ...
                if (this.config.show_day_totals === true) {
                    ...
                    columnsToReturn.forEach((column, index) => {
                        ...
                        if (columnsWithTotals.includes(column) && column !== 'limit-column') {
                            let returnTotal;
                            if (dayTotal[column] === undefined)
                                throw new Error("dayTotal undefined");
        ...
        if (this.config.show_totals === true || this.config.show_plan_totals === true) {
            ...
            columnsToReturn.forEach((column, index) => {
                ...
                if (columnsWithTotals.includes(column) && column !== 'limit-column') {
                    let returnTotal;
                    if (overallTotal[column] === undefined)
                        throw new Error("dayTotal undefined");
                    if (column === "cost-column" || column === "total-column") {
        ...
        if (this.content === null)
            throw new Error("this.content not set");
    ...
    getTimeframeForOverride(timeString) {
        ...
        if (prefix === "sensor")
            prefix = "predbat";
        if (this._hass === undefined)
            throw new Error("this._hass not set");
        ...
        if (match[2] === undefined)
            throw new Error("match[2] undefined");
        let hours = parseInt(match[2], 10);
        if (match[3] === undefined)
            throw new Error("match[3] undefined");
            ...
            if (key === "soc") {
                iconEl.addEventListener('click', () => {
                    ...
                    if (isSocActive) {
                        ...
                        if (snowflakeIcon)
                            snowflakeIcon.style.color = snowIconColorOff;
                    }
                    else {
                        if (fromPopup) {
                ...
                iconEl.addEventListener('click', () => {
                        ...
                        if (parent) {
                            ...
                            allButtons.forEach((btn) => {
                                if (!(btn instanceof HTMLElement))
                                    throw new Error("btn is not HTMLElement");
                    ...
                    if (isAllowed) {
                        if (isActive) {
                            ...
                            if (snowflakeIcon)
                                snowflakeIcon.style.color = snowIconColorOn;
                            this._hass.callService('select', 'select_option', {
                                entity_id: entityObject.entityName,
    ...
    getCellTransformationRefactor(theItem, column, darkMode, itemIndex, timestamp) {
        ...
        if ((this.config.old_skool || this.config.old_skool_columns?.includes(column)) && !isNonDataColumn) {
            if (!isBothField) {
                if (theItem.color === null)
                    throw new Error("theItem.color is null");
        ...
        if (columnsWithCustomTransformation.includes(column)) {
            ...
            if (column === "time-column") {
                ...
                if (column === "time-column" && !hasNonData) {
                    ...
                    newCell.addEventListener('click', () => {
                        const timeframeForOverride = this.getTimeframeForOverride(timestamp.value);
                        if (timeframeForOverride === null)
                            throw new Error("timeframeForOverride is null");
            ...
            if (column === "options-popup-column" || column === "options-column") {
                if (timeForSelectOverride === null)
                    throw new Error("timeForSelectOverride is null");
            ...
            if (column === "import-export-column") {
                if (Array.isArray(theItem) === false)
                    throw new Error("theItem is not an array");
                let newPills = "";
                ...
                theItem.forEach((item, index) => {
                    ...
                    if (this.config.debug_prices_only === true) {
                        ...
                        if (contentWithoutTags === null)
                            throw new Error("contentWithoutTags is null");
            ...
            if (column === "import-column" || column === "export-column") {
                ...
                if (useOldSkool) {
                    if (hasDebug && useDebug)
                        ...
                        if (hasBoldTags)
                            rawValue = wrap(rawValue, 'b');
                        if (hasItalicTags)
                            rawValue = wrap(rawValue, 'i');
                    ...
                    if (isBothField) {
                        ...
                        for (const arrow of arrowsToReturn) {
                            ...
                            if (iconString === undefined)
                                throw new Error("iconString is undefined");
                        ...
                        if (stateText === "Both") {
                            ...
                            if (arrowArray[0] === undefined)
                                throw new Error("arrowArray[0] is undefined");
                            if (arrowArray[1] === undefined)
                                throw new Error("arrowArray[1] is undefined");
            ...
            if (column === "weather-column") {
                if (theItem.value !== undefined && theItem.value !== null) {
                    ...
                    if (this._hass === undefined)
                        throw new Error("_hass undefined");
                    if (this.config.weather_entity === undefined)
                        throw new Error("this.config.weather_entity is undefined");
                    const weatherEntity = this._hass.states[this.config.weather_entity];
                    const readableCondition = this._hass.formatEntityState?.(weatherEntity, condition) ||
    ...
    getPricesFromPriceString(thePriceString, hasBoldTags, hasItalicTags, debugOnly) {
        ...
        if (testMatches === null)
            throw new Error("testMatches is null");
        if (testMatches[1] === undefined)
            throw new Error("testMatches[1] is undefined");
        if (testMatches[2] === undefined)
            throw new Error("testMatches[2] is undefined");
    ...
    getTransformedCostToPill(theItem, darkMode) {
        if (theItem.value === undefined)
            throw new Error("theItem.value is undefined");
    ...
    isLabelDuringNight(label, hass) {
        ...
        if (labelTimeStr === undefined)
            throw new Error("labelTimeStr is undefined");
        const [labelHour, labelMinute] = labelTimeStr.split(':').map(Number);
        if (labelHour === undefined)
            throw new Error("labelHour is undefined");
        if (labelMinute === undefined)
            throw new Error("labelMinute is undefined");
    ...
    findForecastForLabel(label, forecastArray) {
        ...
        if (labelDayStr === undefined)
            throw new Error("labelDayStr is undefined");
        if (invalidDay(labelDayStr))
    ...
    previous_findForecastForLabel(label, forecastArray) {
        ...
        const isCurrentHour = now.getFullYear() === labelDate.getFullYear() &&
    ...
    getArrayDataFromRaw(raw, hassDarkMode) {
        ...
        for (const row of raw.rows) {
            ...
            const stateArrow = Object.entries(arrowGroups).find(([, states]) => states.includes(row.state))?.[0] || "";
            const costArrow = row.cost_change > 0 ? "↗" :
                row.cost_change < 0 ? "↘" :
                    "→";
            const socArrow = row.soc_change > 0 ? "↗" :
                row.soc_change < 0 ? "↘" :
                    "→";
            ...
            if (row.pv_forecast === null)
                throw new Error("row.pv_forecast is null");
            if (row.car_charging === undefined)
                throw new Error("row.car_charging is undefined");
            if (row.iboost === undefined)
                throw new Error("row.iboost is undefined");
        ...
        trElements.forEach((trElement, index) => {
            ...
            if (index > firstRowData && index < (trElements.length - 1)) {
                ...
                tdElements.forEach((tdElement, tdIndex) => {
                    ...
                    if (bgColor === null)
                        throw new Error("bgColor is null");
                    ...
                    if (headerClassesArray[headerIndex] === "total-column") {
                        ...
                        if (temp === undefined)
                            throw new Error("headerClassesArray[headerIndex] is undefined");
                    ...
                    if (Object.keys(newTRObject).length < Object.keys(headerClassesArray).length) {
                        if (headerClassesArray[3] === undefined)
                            throw new Error("headerClassesArray[3] is undefined");
                        if (headerClassesArray[4] === undefined)
                            throw new Error("headerClassesArray[4] is undefined");
                        if (newDataObject.length === 0)
                            throw new Error("newDataObject is empty");
                        const prevRow = newDataObject[newDataObject.length - 1];
                        if (prevRow === undefined)
                            throw new Error("prevRow is undefined");
                        ...
                        if (limitValue === undefined)
                            throw new Error("limitValue is undefined");
                    ...
                    if (headerClassesArray[1] === undefined)
                        throw new Error("headerClassesArray[1] is undefined");
                    if (headerClassesArray[2] === undefined)
                        throw new Error("headerClassesArray[2] is undefined");
                    ...
                    if (importValue === undefined)
                        throw new Error("importValue is undefined");
                    if (exportValue === undefined)
                        throw new Error("exportValue is undefined");
                    newTRObject["import-export-column"] = [importValue, exportValue];
                    // weather forecast
                    if (this.forecast) {
                        let weatherColor = "#FFFFFF"; // var(--primary-text-color)
                        if (newTRObject["time-column"] === undefined)
                            throw new Error("newTRObject['time - column'] is undefined");
                        const match = this.findForecastForLabel(newTRObject["time-column"].value, this.forecast);
                        if (match !== undefined && match !== null) {
                            let matchStore = match;
                            if (this._hass === undefined)
                                throw new Error("this._hass is undefined");
                    ...
                    let iBoostValue = "0";
                    if (pvIndex !== -1) {
                        if (headerClassesArray[pvIndex] === undefined)
                            throw new Error("headerClassesArray[pvIndex] is undefined");
                        const tempNewTRObject = newTRObject[headerClassesArray[pvIndex]];
                        if (tempNewTRObject === undefined)
                            throw new Error("newTRObject[headerClassesArray[pvIndex]] is undefined");
                        pvValue = tempNewTRObject.value.replace(/[☀]/g, '');
                        if (pvValue.length === 0 || Number.isNaN(parseFloat(pvValue)))
                            pvValue = "0";
                    }
                    if (loadIndex !== -1) {
                        if (headerClassesArray[loadIndex] === undefined)
                            throw new Error("headerClassesArray[loadIndex] is undefined");
                    ...
                    if (carIndex !== -1) {
                        if (headerClassesArray[carIndex] === undefined)
                            throw new Error("headerClassesArray[carIndex] is undefined");
                        const tempNewTRObject = newTRObject[headerClassesArray[carIndex]];
                        if (tempNewTRObject === undefined)
                            throw new Error("newTRObject[headerClassesArray[carIndex]] is undefined");
                        carValue = tempNewTRObject.value;
                        if (carValue.length === 0 || Number.isNaN(parseFloat(carValue)))
                            carValue = "0";
                    }
                    if (iBoostIndex !== -1) {
                        if (headerClassesArray[iBoostIndex] === undefined)
                            throw new Error("headerClassesArray[iBoostIndex] is undefined");
                        const tempNewTRObject = newTRObject[headerClassesArray[iBoostIndex]];
                        if (tempNewTRObject === undefined)
                            throw new Error("newTRObject[headerClassesArray[iBoostIndex]] is undefined");
    ...
    getStringToDate(input) {
        ...
        if (hours === undefined)
            throw new Error("hours is undefined");
...
function getPrefix(entity) {
    ...
    if (prefixes === null)
        throw new Error("prefixes not set");
    return prefixes[0];
}
function invalidDay(labelDayStr) {
    if (!["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].includes(labelDayStr)) {
        return true;
    }
    return false;
...
export {};
//# sourceMappingURL=predbat-table-card.js.map